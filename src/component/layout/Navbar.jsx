...
export const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 right-0 z-1000 w-full py-4 transition-all duration-300 ${isScrolled
        ...
        >
          {
            NAV_LINKS.map((link)=>(
              ...
              key={link.id}
              onClick={()=>handleNavClick(link.id)}
              className={`block w-full text-left px-5 py-3 rounded-lg font-medium transition-all duration-300 ${activeSection===link.id
          ...
          <button 
           onClick={()=>handleNavClick('contact')}
           className='w-full px-7 py-3.5 bg-white text-[#212121] font-medium text-base rounded-[17px] border border-white hover:bg-white/90 transition-all duration-300 mt2'
          >
            Hire Me
          </button>
        </div>
