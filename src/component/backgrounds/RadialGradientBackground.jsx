...
const RadialGradientBackground=({variant='hero', gradients=[]})=>{
    ...
    const generatedGradient = (colors) =>{
        // build a valid radial-gradient string. colors array may only contain color values,
        // so join them and use surrounding transparent stops to create soft edges.
        const colorList = colors.map(({color, stop})=> `${color} ${stop}`).join(', ');
        return `radial-gradient(circle at center, transparent 0%, transparent 30%, ${colorList}, transparent 60%, transparent 100%)`;
    }
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none" >
            ...
            {activeGradients.map((gradient,index)=>(
                <div
                    ...
                    style={{ background: generatedGradient(gradient.colors),
