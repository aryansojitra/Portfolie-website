...
export const ProjectCard = ({ project }) => {
  const { title, description, image, technologies, demoUrl, metrics, githubUrl } = project;
  ...
  return (
    <div className='group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300'>
        ...
        <div className='absolute bottom-4 right-4 flex items-center gap-3'>
          {demoUrl && (
            <a 
              ...
              className='p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-primary/50 transition-all duration-300 hover:scale-110'
              title='View Demo'
              >
      ...
      <div className='p-6 space-y-4'>
        <div>
          <h3 className='text-xl font-bold text-white mb-2 group-hover:text-[#A8FF8D] transition-colors duration-300'>
            {title}
          </h3>
          ...
          <div className='flex flex-wrap gap-2'>
            {technologies.map((tech,index)=>(
              <span
                key={index}
                className='px-3 py-1 text-xs font-medium text-primary bg-primary/10 border border-white/20 rounded-lg hover:bg-primary/20 transition-colors duration-300'
              >
                {tech}
              </span>
            ))}
